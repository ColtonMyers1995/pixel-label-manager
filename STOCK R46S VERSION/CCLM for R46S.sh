#!/bin/bash
# Pixel Toolkit FINAL - R46S Stable Menu Build

export TERM=linux
export NCURSES_NO_UTF8_ACS=1

# --- R46S THEME DETECTION ---
if [ -d "/STORAGE/.emulationstation/themes/es-theme-PIXEL-OS" ]; then
    THEME_DIR="/STORAGE/.emulationstation/themes/es-theme-PIXEL-OS"
elif [ -d "/storage/.emulationstation/themes/es-theme-PIXEL-OS" ]; then
    THEME_DIR="/storage/.emulationstation/themes/es-theme-PIXEL-OS"
elif [ -d "/roms/themes/es-theme-PIXEL-OS" ]; then
    THEME_DIR="/roms/themes/es-theme-PIXEL-OS"
else
    dialog --ascii-lines --msgbox "Could not locate PIXEL OS theme folder." 8 60
    clear
    exit 1
fi
...
# --- R46S CONTROLLER SUPPORT ---
. /etc/profile

cat > /tmp/pixeltoolkit.gptk << 'EOF'
js0:a = enter
js0:b = esc
EOF

gptokeyb "dialog" -c /tmp/pixeltoolkit.gptk >/dev/null 2>&1 &

if command -v jslisten >/dev/null 2>&1; then
    jslisten set "PixelToolkit"
fi


safe_msgbox(){ dialog --ascii-lines --msgbox "$1" 7 60 > "$CURR_TTY"; }

mkdir -p "$THEME_DIR/art"
mkdir -p "$THEME_DIR/art2"
mkdir -p "$THEME_DIR/art3"
...
# ---------------- UPDATE (TOOL + THEME) ----------------
UpdateToolkit(){
dialog --ascii-lines --yesno "Update Toolkit + Theme from GitHub?\nRequires internet connection." 8 60 || return
...
TOOL_PATH="/roms/ports/Custom-Collection Label-Maker.sh"
...
while true; do
DISPLAY="${TEXT:-_}"
CHOICE=$(dialog --ascii-lines --output-fd 1 --menu "$DISPLAY" 20 60 10 \
...
select_layout(){
dialog --ascii-lines --output-fd 1 --menu "Label Layout" 15 60 3 \
...
select_icon_source(){
SRC=$(dialog --ascii-lines --output-fd 1 --menu "Select Source" 18 60 4 \
1 "Icons (art2)" \
...
if [ ! -d "$ICON_DIR" ]; then
safe_msgbox "Directory not found:
...
if [ ${#FILES[@]} -eq 0 ]; then
safe_msgbox "No PNG files found in:

$ICON_DIR"
return 1
fi

IDX=$(dialog --ascii-lines --output-fd 1 --menu "Select PNG" 20 60 12 "${MENU[@]}" 2>"$CURR_TTY") || return 1
echo "${FILES[$IDX]}"
}

select_size(){
S=$(dialog --ascii-lines --output-fd 1 --menu "Image Size" 12 40 3 \
...
select_text_size(){
S=$(dialog --ascii-lines --output-fd 1 --menu "Text Size" 12 40 3 \
1 "Small" 2 "Medium" 3 "Big" \
2>"$CURR_TTY") || return 1
case $S in
1) echo 24 ;;
...
select_spacing(){
S=$(dialog --ascii-lines --output-fd 1 --menu "Icon/Text Spacing" 12 40 3 \
...
select_font(){
F=$(dialog --ascii-lines --output-fd 1 --menu "Font" 30 70 21 \
...
select_color(){
C=$(dialog --ascii-lines --output-fd 1 --menu "Text Color" 25 60 15 \
...
select_padding(){
P=$(dialog --ascii-lines --output-fd 1 --menu "Horizontal Padding" 12 40 3 \
...
select_vpadding(){
P=$(dialog --ascii-lines --output-fd 1 --menu "Vertical Padding" 12 40 3 \
1 "More Padding" 2 "Normal" 3 "Less Padding" \
2>"$CURR_TTY") || return 1
case $P in
1) echo 25 ;;
...
RestartES(){ sync; reboot; }

while true; do
CH=$(dialog --ascii-lines --output-fd 1 --menu "Pixel Toolkit\n\nD-Pad - Scroll | Start - Enter | Select - Back" 17 70 3 \
1 "Makers" \
2 "Settings" \
3 "Exit" \
2>"$CURR_TTY") || continue
...
1)
SUB=$(dialog --ascii-lines --output-fd 1 --menu "Makers" 15 60 3 \
1 "Logo Maker" \
2 "Icon Maker" \
3 "Image Maker" \
2>"$CURR_TTY") || continue
...
2)
SUB=$(dialog --ascii-lines --output-fd 1 --menu "Settings" 17 60 4 \
1 "Not Available" \
2 "Not Available" \
...
killall gptokeyb >/dev/null 2>&1 || true
